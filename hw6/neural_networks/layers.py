...
class FullyConnected(Layer):
    ...
    def _init_parameters(self, X_shape: Tuple[int]) -> None:
        ...
        b = np.zeros((X_shape[0],W.shape[1]))

        self.parameters = OrderedDict({"W": W, "b": b})
        self.cache: OrderedDict = OrderedDict()
        self.gradients: OrderedDict = OrderedDict({"W":0,"b":0})
        # initialize parameter gradients to zeros
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        affine = np.dot(X,self.parameters["W"]) + self.parameters["b"]
        out = self.activation(affine)
        # store information necessary for backprop in `self.cache`
        self.cache["X"] = X
        self.cache["W"] = self.parameters["W"]
        self.cache["b"] = self.parameters["b"]
        self.cache["out"] = affine
        ### END YOUR CODE ###
    ...
    def backward(self, dLdY: np.ndarray) -> np.ndarray:
        ...
        b = self.cache["b"]
        out = self.cache["out"]
        dy = self.activation.backward(out,dLdY)
        dX = np.dot(dy,np.transpose(W))
        self.gradients["W"] = np.dot(np.transpose(X),dy)
        self.gradients["b"] = dy
...
class Elman(Layer):
    ...
    def _init_cache(self, X_shape: Tuple[int]) -> None:
        ...
        s0 = np.zeros((X_shape[0],self.parameters["U"].shape[1]))# the first hidden state
        self.cache = OrderedDict({"s": [s0],"X":[],"Z":[]})
    ...
    def backward(self, dLdY: np.ndarray) -> List[np.ndarray]:
        ...
        for i in range(len(Z)-1,-1,-1):
            dLdZ = self.activation.backward(Z[i],dLdY)
            dXt = np.dot(dLdZ,self.parameters["W"].T)
            dLdX += [dXt]
            
            self.gradients["W"] += np.dot(X[i].T,dLdZ)
            self.gradients["U"] += np.dot(s[i].T,dLdZ)
            self.gradients["b"] += dLdZ
            
            dLdY = np.dot(dLdZ,self.parameters["U"].T)
            ...
            for o in range(out_channels):
                ...
                for hprime in range(out_rows):
                    for wprime in range(out_cols):
                        ...
                        der_new_x[n,starth:endh,startw:endw,:] += W[:,:,:,o]*dldIK[n,hprime,wprime,o]

...
class L2(Loss):
    ...
    def forward(self, Y: np.ndarray, Y_hat: np.ndarray) -> float:
        ...
        """
        ### YOUR CODE HERE ###
        return np.sum(np.square(np.subtract(Y,Y_hat)),axis=0)/Y.shape[0]
    ...
    def backward(self, Y: np.ndarray, Y_hat: np.ndarray) -> np.ndarray:
        ...
        """
        ### YOUR CODE HERE ###
        return (-2)*np.subtract(Y,Y_hat)/Y.shape[0]
